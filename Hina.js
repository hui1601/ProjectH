...
DB.createDir = function() { //배운 채팅들이 저장될 폴더를 만드는 함수
	var folder = new java.io.File(sdcard + "/Hina/"); //File 인스턴스 생성
	folder.mkdirs(); //폴더 생성
};
DB.saveData = function(name, msg) { //파일에 내용을 저장하는 함수
	try { //사실, 나도 어디서 긁어와서 이곳저곳에서 사용하는 거임
		var file = new java.io.File(sdcard + "/Hina/" + name + ".txt");
	...
	switch (cmd) {
		...
		case "/DB":
			if (isGroupChat) {
				...
				if (data == null) Hina.say("0개", r);
				else Hina.say(data.split("\n").length + "개", r);
			}
			break;
	}
}
